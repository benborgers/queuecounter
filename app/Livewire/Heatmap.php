...
class Heatmap extends Component
{
    ...
    private function getEntryFifteenMinuteData()
    {
        ...
        $intervalData = Entry::select([
            DB::raw('EXTRACT(HOUR FROM timestamp AT TIME ZONE \'America/New_York\') AS hour'),
            DB::raw('FLOOR(EXTRACT(MINUTE FROM timestamp AT TIME ZONE \'America/New_York\') / 15) AS quarter_hour'),
            DB::raw('COUNT(*) as count')
        ])
            ->whereRaw('EXTRACT(HOUR FROM timestamp AT TIME ZONE \'America/New_York\') BETWEEN ? AND ?', 
        ...
        if ($this->mode === 'entry') {
            return Entry::select(DB::raw('COUNT(*) as count'))
                ->whereRaw('EXTRACT(HOUR FROM timestamp AT TIME ZONE \'America/New_York\') BETWEEN ? AND ?', 
                    [$this->startHour, $this->endHour])
                ->groupBy([
                    DB::raw('EXTRACT(HOUR FROM timestamp AT TIME ZONE \'America/New_York\')'),
                    DB::raw('FLOOR(EXTRACT(MINUTE FROM timestamp AT TIME ZONE \'America/New_York\') / 15)')
                ])
                ->orderBy('count', 'desc')
                ->value('count') ?? 0;
        } else {
